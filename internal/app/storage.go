...
func initHistoryDB(path string) error {
	...
	stmts := []string{
		...
		`CREATE TABLE IF NOT EXISTS computer_steps (
			...
			operation TEXT NOT NULL DEFAULT 'act',
			arguments_json TEXT NOT NULL DEFAULT '{}',
		...
		`CREATE INDEX IF NOT EXISTS idx_computer_steps_tool_call ON computer_steps(tool_call_id)`,
		`CREATE INDEX IF NOT EXISTS idx_computer_steps_message ON computer_steps(message_id)`,
		`CREATE TABLE IF NOT EXISTS computer_step_checks (
			id TEXT PRIMARY KEY,
			...
			diff_score REAL NOT NULL DEFAULT 0,
			...
			result_json TEXT NOT NULL DEFAULT '{}',
			created_at TEXT NOT NULL,
			FOREIGN KEY(step_id) REFERENCES computer_steps(id)
		)`,
		`CREATE INDEX IF NOT EXISTS idx_computer_step_checks_step ON computer_step_checks(step_id, created_at)`,
