...
func functionPolicyText() string {
	return `Use only the model-visible tools when you need actions. Prefer compact unified tools: db, memory, query, evaluate_turn, sendmsg, selfie, computer, notify, schedule, summarize, dream, and meditate. computer is high-permission desktop observation/control; call computer operation=help before using it for a task, use observe before uncertain coordinates, and ask the user before sensitive, irreversible, account, payment, or deletion actions. If a computer result has ok=false, stop acting and explain the error/diagnosis to the user instead of trying more mouse or keyboard actions. A callback is another tool call; use callback.tool="sendmsg" to send a tool result to the user, to yourself for continuation, or to the internal event stream. user_set_profile is read-only; write only your own estimated user profile. Memory IDs are shown as M<number>. Mark a memory as used only when you actually relied on it in this reply; recalled memories are not automatically used.
